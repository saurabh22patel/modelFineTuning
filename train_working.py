...
def barrier_with_timeout(timeout_seconds=300):
    ...
    try:
        ...
        if thread.is_alive():
            raise RuntimeError(f"Barrier timed out after {timeout_seconds} seconds")
...
def signal_handler(signum, frame):
    ...
    print(f"\n[Rank {rank}] Received signal {signum}, initiating graceful shutdown...", flush=True)
    ...
    try:
        if is_initialized:
            ...
            try:
                ...
                pass  # Expected if ranks are still initializing
...
def setup_distributed():
    ...
    if "SLURM_PROCID" in os.environ:
        ...
        if "MASTER_ADDR" not in os.environ:
            raise ValueError("MASTER_ADDR not set in environment! Check SLURM script.")
...
def prepare_dataset(dataset_path, tokenizer, max_length):
    ...
    if "input_ids" in dataset.column_names and "labels" in dataset.column_names:
        print("Dataset is already pre-tokenized.", flush=True)
        return dataset
...
class CustomTrainer(Trainer):
    ...
    def _inner_training_loop(self, *args, **kwargs):
        ...
        if self._model_already_fsdp and hasattr(self, 'accelerator') and self.accelerator is not None:
    ...
    def log(self, logs, start_time=None):
        ...
        try:
            ...
            if is_rank_0 and mlflow.active_run() is not None:
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        if eval_dataset is None:
            if dist.is_initialized() and dist.get_rank() == 0:
                print("Skipping evaluation (no eval dataset provided)", flush=True)
            return {}
...
def main():
    ...
    try:
        ...
        print_flush(f"[Rank {rank}] Distributed training initialized - Local rank: {local_rank}, World size: {world_size}", rank=0)
    except Exception as e:
    ...
    if torch.cuda.is_available():
        ...
        gpu_name = torch.cuda.get_device_properties(actual_gpu_index).name
        total_memory = torch.cuda.get_device_properties(actual_gpu_index).total_memory / 1e9
        print_flush(f"[Rank {rank}] GPU {actual_gpu_index} ({gpu_name}) - {total_memory:.2f} GB", rank=rank)
    ...
    if rank == 0:
        tracking_uri = config["mlflow"].get("tracking_uri") or os.environ.get("MLFLOW_TRACKING_URI", "file:./mlruns")
        ...
        print(f"Experiment: {config['mlflow']['experiment_name']}, Run: {run_name}")
    ...
    if hf_token:
        try:
            ...
            print_flush(f"WARNING: HuggingFace login failed: {e}", rank=rank)
    ...
    try:
        ...
        use_token = hf_token if (hf_token and (not is_local_path or not path_exists)) else None
        tokenizer = AutoTokenizer.from_pretrained(model_path, token=use_token, fix_mistral_regex=True)
        if tokenizer.pad_token is None:
            tokenizer.pad_token = tokenizer.eos_token
        print_flush("Tokenizer loaded successfully", rank=0)
    except Exception as e:
        print_flush(f"ERROR loading tokenizer: {e}", rank=0)
        traceback.print_exc()
        cleanup_distributed()
        force_cleanup_and_exit(1)
    
    try:
        print_flush("Loading model (this may take several minutes)...", rank=0)
        model_path = config["model"]["path"]
        is_local_path = os.path.isabs(model_path) or model_path.startswith('./') or model_path.startswith('../')
        path_exists = os.path.exists(model_path) if is_local_path else False
        ...
        if use_flash_attention:
            try:
                ...
                if rank == 0:
                    print("Flash Attention 2 enabled", flush=True)
            except ImportError:
                if rank == 0:
                    print("WARNING: Flash Attention requested but not installed. Using default attention.", flush=True)
        ...
        print_flush(f"ERROR loading model: {e}", rank=0)
        traceback.print_exc()
        cleanup_distributed()
        force_cleanup_and_exit(1)
    ...
    try:
        ...
        print_flush(f"ERROR preparing dataset: {e}", rank=0)
        traceback.print_exc()
        cleanup_distributed()
        force_cleanup_and_exit(1)
    ...
    try:
        ...
        if rank == 0:
            print(f"Error detecting transformer layer class: {e}", flush=True)
    ...
    if transformer_layer_cls is not None:
        ...
        if rank == 0:
            print(f"Using transformer_auto_wrap_policy with {transformer_layer_cls.__name__}", flush=True)
    else:
        from torch.distributed.fsdp.wrap import size_based_auto_wrap_policy
        auto_wrap_policy = partial(size_based_auto_wrap_policy, min_num_params=1e7)
        if rank == 0:
            print("Using size-based auto_wrap_policy", flush=True)
    
    if rank == 0:
        print("Wrapping model with FSDP...", flush=True)
    ...
    if rank == 0:
        print("Model wrapped with FSDP successfully", flush=True)
    ...
    try:
        if dist.is_initialized():
            print_flush("Verifying all ranks are ready...", rank=0)
            try:
                barrier_with_timeout(timeout_seconds=120)
                print_flush("All ranks synchronized, starting training...", rank=0)
            except Exception as e:
                print_flush(f"WARNING: Barrier failed: {e}. Proceeding anyway...", rank=0)
        ...
        if _shutdown_requested:
            print_flush("Shutdown requested before training start", rank=0)
            return
        ...
        try:
            ...
            print_flush(f"Dataloader test successful. Batch keys: {list(first_batch.keys())}", rank=0)
        except Exception as e:
            print_flush(f"ERROR loading first batch: {e}", rank=0)
            traceback.print_exc()
            raise
        ...
        print_flush("Training completed successfully!", rank=0)
        
        if _shutdown_requested:
            print_flush("Shutdown requested after training", rank=0)
            return
        ...
        if rank == 0 and training_successful:
            ...
            try:
                if trainer.gpu_utilizations:
                    ...
                    print(f"\nGPU Utilization Stats: Avg={avg_util:.2f}%, Min={min_util:.2f}%, Max={max_util:.2f}%", flush=True)
            except Exception as e:
